...
use mysql::{Conn, prelude::Queryable, params};
...
#[tauri::command]
 async fn get_all_task(state: State<'_, DBConnection>) ->Result<String,String>{
    ...
    db.conn.exec_map("SELECT id,text,complete FROM task
...
async fn update_the_task(id:i32,complete:i32,state: State<'_, DBConnection>)->Result<(),()>{
    let mut db = state.0.lock().await;
    let stmt =
     db.conn.prep("update task set complete=:complete where id=:id").unwrap();
    let res =
...
async fn dalete_the_task(id:i32,state:State<'_, DBConnection>)->Result<(),()>{
    let mut db = state.0.lock().await;
    let stmt = 
        db.conn.prep("delete from task where id=:id").unwrap();
    let res = 
        db.conn.exec_drop(&stmt, params!{
            "id"=>id
        });
        ...
        .manage(
            DBConnection(
                Mutex::new(
                    InnerDBConnection{
                        conn:
                            Conn::new("mysql://root:root@127.0.0.1:3306/db_todos?prefer_socket=false")
                            .unwrap()
                    }
                )
            )
        )
        .invoke_handler(tauri::generate_handler![get_all_task,insert_new_task,update_the_task,dalete_the_task])
